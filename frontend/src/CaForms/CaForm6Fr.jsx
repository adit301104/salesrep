...
const CaForm6Fr = () => {
  ...
  const [previewLicenserSignature, setPreviewLicenserSignature] = useState(null);
  const [previewClientSignature, setPreviewClientSignature] = useState(null);
  const licenserSignatureRef = useRef(null);
  ...
  const handleSignatureUpload = (e, type) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        if (type === 'licenser') {
          setPreviewLicenserSignature(reader.result);
        } else {
  ...
  const triggerSignatureUpload = (type) => {
    if (type === 'licenser') {
      licenserSignatureRef.current.click();
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const submissionData = {
        formType: "CaForm6Fr",
        title: `Contrat de Service Concessionnaire - ${formData.clientLegalName}`,
        fields: {
          ...formData,
          licenserSignature: previewLicenserSignature,
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto bg-white">
      ...
      <input
        type="file"
        ref={licenserSignatureRef}
        onChange={(e) => handleSignatureUpload(e, 'licenser')}
        accept="image/*"
        className="hidden"
      />
      <input
      ...
      <div className="flex justify-between items-start mb-8">
        ...
        <div className="text-right">
          <h1 className="text-2xl font-bold">CONCESSIONNAIRE CONTRAT DE SERVICE</h1>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        {/* Informations du licensier */}
        <div className="space-y-4 mb-8">
          <div className="flex gap-4">
            <div className="flex-1">
              <label className="block mb-1">1) Licensier:</label>
              <input
                type="text"
                name="licenser"
          ...
          <div className="flex gap-4">
            <div className="flex-1">
              <label className="block mb-1">2) Adresse:</label>
              <input
                type="text"
        ...
        <div className="flex gap-8 mb-8">
          ...
          <div className="flex-1">
            <label className="block font-semibold mb-2">Coût mensuel+Taxes:</label>
            <input
              type="text"
              name="monthlyCost"
        ...
        {/* Date des déductions */}
        <div className="mb-8">
          <label className="block font-semibold mb-2">Date des déductions mensuels:</label>
          <input
            type="text"
            name="deductionDate"
        ...
        <div className="mb-8">
          ...
          <div className="space-y-2 text-sm bg-gray-50 p-4 rounded-lg">
            <ol className="list-decimal pl-5 space-y-2">
              <li>La Corporation Autowiz est d'accord d'octroyer au client ci-haut nommé le code d'accès afin télécharger depuis www.autowizapp.com le logiciel de la plateforme automobile.</li>
              <li>Le client accepte de payer la somme ci-dessus sur une base mensuelle par carte de crédit pour la durée décrite ci-dessus.</li>
              <li>Il n'y a aucun remboursement disponible sur ce contrat.</li>
              <li>Le client accepte de payer des frais de service de deux fois les frais bancaires si le débit mensuel est refusé.</li>
              <li>Si le client souhaite résilier ce contrat, des frais équivalant à deux mois de débit seront débités au client sur la forme de paiement fourni par le client, ce que le client accepte de payer.</li>
              <li>Ce contrat se renouvelle automatiquement pour une durée équivalente sauf si un avis est reçu du client 60 jours avant l'expiration de son intention de ne pas procéder à un renouvellement.</li>
            </ol>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-8 mb-8">
          <div className="flex-1">
            <h3 className="font-medium mb-2">Signature du licensier</h3>
            <div className="border-t-2 border-gray-400 pt-2 mb-4 h-40 flex items-center justify-center bg-gray-50">
              {previewLicenserSignature ? (
                <img 
                  src={previewLicenserSignature} 
            ...
            <button
              type="button"
              onClick={() => triggerSignatureUpload('licenser')}
            ...
            <p className="text-sm mt-2 text-center">Signature du Licensier</p>
          </div>
          ...
          <div className="flex-1">
            ...
            <p className="text-sm mt-2 text-center">Signature du Client</p>
